...
## RELIABILITY FIXES (post round-2, test-locked) — 35/35 green, deterministic
- **db-layer 2-hop / var-ref export resolution:** query fns exported as `module.exports.x = localConstFn` (and fns that call other local fns) were losing their writes → controllers showed reads-only → actor-model fell back to a read model → WRONG entity/predicate. Now resolves local-const exports + inlines transitive local-fn calls. General blast-radius accuracy win (e.g. updateAttendeesList now shows User.create*/SessionParticipant.deleteMany* etc).
- **Route-aware primary_entity:** the ownership predicate is now scoped to the object the URL names (`/sessions/:sessionId/... → sessions`), preferring a confirmed entity matching a URL segment over an arbitrary written model. Fixes `attendeesList` emitting `users.employerOrganisation` on a session route → now `sessions.runByTrainingProvider == req.user.tpOrganisation`. Locked with a test invariant. 3 live IDORs + updateUserById unchanged.
- **`const {user}=req` gate detection + forbid-enclosure:** backend-actors' in-controller gate now resolves `const {user}=req` destructuring (gwent's idiom) AND requires the role check to actually forbid/return on failure (not a behaviour branch like `else if (role===trainer){push}`). gwent controllerGated 3→9 (sound), false CSOA on admin-gated routes (getAllParticipants/restore/my-trainers) removed; add-session correctly stays a CSOA candidate.
- **Skipped deliberately:** guardRoles default-deny coverage (latent, not firing today; a wrong fix = a missed IDOR/FN, worse than the latent FP). Documented open.
